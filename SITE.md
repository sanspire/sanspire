...
- Work (`/work`) - Portfolio listing; each **whole card** is a link to **`/work/{slug}`** (no scroll-in animation on this page). Project slugs match the case study routes (see defaults for the full list).
- Case study pages (`/work/[slug]`) - Shared layout: dark hero (on **mobile**, text block **above** the hero image; **desktop** image left, copy right), cream **challenge** and **solution** copy, then a gallery with light hover zoom on images; defaults in **`lib/caseStudyProjectDefaults.ts`**. No separate “The result” block or wide cover image.
